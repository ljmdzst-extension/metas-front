  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (currentToken && user) {
        ...
        const isTokenExpired = tokenExpiry && new Date().getTime() > new Date(parseInt(tokenExpiry, 10)).getTime();
        const renewWindowStart = tokenExpiry && new Date(parseInt(tokenExpiry, 10)).getTime() - RENEW_WINDOW_MS;
        const isInRenewWindow = renewWindowStart !== null && new Date().getTime() > Number(renewWindowStart);
